...
This flow chart illustrates how Pharia Kernel is published and deployed.

```mermaid
flowchart LR
    A(New commit) -->|builds| B("New image<br/>Tag: $DATE-$COMMIT_ID")
    B --> C("New Pharia Kernel deployment (canary)<br/><br/>Cluster: c-aa01<br/>Chart version: latest<br/>App version: latest<br/>Tag: $DATE-$COMMIT_ID<br/>or<br/>$APP_VERSION")
    D(New tag) -->|builds| E("New image<br/>Tag: $APP_VERSION")
    E --> C
    E --> F(New Pharia Kernel chart)
