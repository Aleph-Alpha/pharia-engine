...
/// If you choose, for example, [`Granularity::Sentence`] then we report the importance score of
/// each sentence in the prompt towards generating the target output.
...
/// While `[aleph_alpha_client::ExplanationOutput]` contains multiple items for `Text`, `Image`, and
/// `Target`, we do not support multi-modal prompts and do not return any scores for `Image`.
/// As we also do not support target-granularity as part of the `[crate::ExplanationRequest]`, we
/// will get an empty vector in the target scores, and therefore can ignore these one as well.
...
///
/// A known limitation is that we take assumptions about the thinking tokens of the model (qwen
/// syntax). However, since reasoning models have not been available in the Aleph Alpha API for a
/// long time, it is reasonable to assume that the only reasoning model used inside skills is qwen
/// (if even).
...
pub enum ChatEvent {
    ...
    MessageAppend {
        ...
        content: String,
        /// Log probabilities of the completion tokens if requested via logprobs parameter in
        /// request.
        logprobs: Vec<Distribution>,
    },
...
pub struct Distribution {
    ...
    pub sampled: Logprob,
    // Logarithmic probabilities of the most probable tokens, filled if user has requested
    // [`crate::Logprobs::Top`]
    pub top: Vec<Logprob>,
}
    ...
    async fn act(self, client: &impl InferenceClient, gen_ai_content_capture: bool) {
        match self {
            ...
            Self::ChatStreamV2 {
                ...
                let (event_send, mut event_recv) = mpsc::channel(1);
                let mut stream =
                    Box::pin(client.stream_chat_v2(&request, auth, &context, event_send));
